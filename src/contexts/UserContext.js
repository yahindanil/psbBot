...
// Функция для создания тестового пользователя при локальной разработке
// Имитирует пользователя Telegram с данными John Doe для работы с продакшн бекендом
...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      if (isLocalDev) {
        log(
          "🛠 ЛОКАЛЬНАЯ РАЗРАБОТКА: Подключение к продакшн бекенду с тестовым пользователем"
        );

        // Создаем тестового пользователя John Doe для локальной разработки
        ...
        // Автоматически создаем/получаем пользователя в продакшн БД
        try {
          log("Создание/получение пользователя John Doe в продакшн БД...");
          ...
          log(
            "Пользователь John Doe успешно создан/получен в продакшн БД",
            dbUserData
          );
          ...
          log("Ошибка при работе с продакшн БД (пользователь John Doe)", error);
